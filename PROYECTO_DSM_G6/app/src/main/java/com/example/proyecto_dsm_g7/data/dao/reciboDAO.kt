...
class reciboDAO {
    ...
    fun getRecibo(id: Int, fechaI: String, fechaF: String, idEstado: Int) : List<recibo> {
        val result = mutableListOf<recibo>()

        val idEstado = idEstado
        ...
        transaction {
            ...
            val query = recibo_table.select((recibo_table.idCasa eq id) and (recibo_table.idReciboEstado eq idEstado) and(recibo_table.fechaEmision.between(fechaInicio,fechaFin)))
            ...
            query.forEach {
                ...
                result.add(
                    recibo(
                        idMantenimientoRecibo, idCasa, nRecibo, periodo, fechaEmision, fechaVencimiento, importe, ajuste, observacion, idReciboEstado,
                    )
                )
            }
        }
